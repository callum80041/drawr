...
export async function POST(req: NextRequest) {
  ...
  if (assignErr) {
    console.error('[reset-draw] assignments delete failed:', assignErr)
    return NextResponse.json({ error: assignErr.message }, { status: 500 })
  }

  // Reset draw fields on the sweepstake
  ...
  if (updateErr) {
    console.error('[reset-draw] sweepstake update failed:', updateErr)
