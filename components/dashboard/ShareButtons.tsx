...
export function ShareButtons({ url, sweepstakeName, isEurovision = false, size = 'sm' }: Props) {
  ...
  const tournamentLabel = isEurovision ? 'Eurovision 2026 sweepstake' : 'World Cup 2026 sweepstake'
  const shareText = `Join my ${tournamentLabel} — ${sweepstakeName}! Sign up here:`
  ...
  return (
    <div className={size === 'lg' ? '' : 'mt-3'}>
      {size === 'sm' && <p className="text-xs text-mid mb-2 font-medium">Share via</p>}
      <div className="flex flex-wrap gap-2">
        {/* Copy link */}
        <button
          type="button"
          onClick={handleCopy}
          className={`${btnBase} ${copied ? 'bg-lime/20 border-lime/40 text-pitch' : 'bg-white border-[#D1D9D5] text-pitch hover:border-grass'}`}
        >
          {copyIcon}
        ...
        <a
          ...
          className={`${btnBase} bg-white border-[#D1D9D5] text-[#25D366] hover:border-[#25D366]`}
        >
          {whatsappIcon}
          WhatsApp
        </a>
        ...
        <a
          ...
          className={`${btnBase} bg-white border-[#D1D9D5] text-mid hover:border-mid hover:text-pitch`}
        >
          {emailIcon}
          Email
        </a>
