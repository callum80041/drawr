...
export function notificationLeaderboardChangeEmailHtml({
  participantName,
  sweepstakeName,
  shareToken,
  leaderName,
...
<title>New leader in ${sweepstakeName}</title>
</head>
      ...
      <tr>
        <td style="padding:32px;">
          <p style="margin:0 0 8px;font-size:20px;font-weight:700;color:#1A2E22;letter-spacing:-0.3px;">
            Someone's taken the lead 🏆
          </p>
          <p style="margin:0 0 24px;font-size:15px;color:#5A7265;line-height:1.6;">
            <strong style="color:#1A2E22;">${escapeHtml(leaderName)}</strong> is now at the top of the leaderboard for <strong style="color:#1A2E22;">${escapeHtml(sweepstakeName)}</strong>.
            The race is on.
          </p>
          ...
          <table cellpadding="0" cellspacing="0" style="width:100%;background:${sectionBg};border-radius:12px;margin-bottom:24px;">
            <tr>
              <td style="padding:16px 20px;">
                <p style="margin:0 0 8px;font-size:11px;font-weight:600;color:#5A7265;text-transform:uppercase;letter-spacing:0.08em;">Leading score</p>
                <p style="margin:0;font-size:28px;font-weight:800;color:#1A2E22;letter-spacing:-0.5px;">${leaderPoints} <span style="font-size:16px;font-weight:600;">pts</span></p>
              </td>
            </tr>
          </table>
