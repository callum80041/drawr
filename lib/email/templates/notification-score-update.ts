...
export function notificationScoreUpdateEmailHtml({
  participantName,
  sweepstakeName,
  shareToken,
  teamName,
...
<title>${teamName} just scored!</title>
</head>
      ...
      <tr>
        <td style="background:#1A2E22;padding:0 32px 32px;text-align:center;">
          ...
          <p style="margin:0;font-size:24px;font-weight:800;color:#C8F04D;letter-spacing:-0.5px;">
            ${teamFlag ?? '🏳️'} ${escapeHtml(teamName)}
          </p>
        </td>
      </tr>
      ...
      <tr>
        <td style="padding:32px;">
          ...
          <p style="margin:0 0 24px;font-size:15px;color:#5A7265;line-height:1.6;">
            <strong style="color:#1A2E22;">${escapeHtml(teamName)}</strong> just picked up ${pointsEarned} point${pointsEarned !== 1 ? 's' : ''} in <strong style="color:#1A2E22;">${escapeHtml(sweepstakeName)}</strong>.
            You're on the board!
          </p>
          ...
          <table cellpadding="0" cellspacing="0" style="width:100%;background:${sectionBg};border-radius:12px;margin-bottom:24px;">
            <tr>
              <td style="padding:16px 20px;">
                <p style="margin:0 0 8px;font-size:11px;font-weight:600;color:#5A7265;text-transform:uppercase;letter-spacing:0.08em;">Your total</p>
                <p style="margin:0;font-size:28px;font-weight:800;color:#1A2E22;letter-spacing:-0.5px;">${totalPoints} <span style="font-size:16px;font-weight:600;">pts</span></p>
              </td>
            </tr>
          </table>
          ...
          <p style="margin:20px 0 0;font-size:12px;color:#8EA899;text-align:center;">
            Keep scoring and you might take the lead.
          </p>
        </td>
      </tr>
